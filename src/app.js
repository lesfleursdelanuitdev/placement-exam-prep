...
(function (G) {
  ...
  const App = { items: [], view: 'home', part: 1, tutId: null, tutTab: 'lesson', returnItem: -1, returnView: 'tutorials', deck: null, deckReturn: 'flashcards', hash: '' };
  G.MX.App = App;
  ...
  function renderExam() {
    ...
    byId('part-nav').innerHTML = (prev ? `<button type="button" class="btn" data-act="goto-part" data-part="${prev}">← Part ${PART_NUM[prev]}: ${esc(PART_NAME[prev])}</button>` : '<span></span>') +
      (next ? `<button type="button" class="btn primary" data-act="goto-part" data-part="${next}">Part ${PART_NUM[next]}: ${esc(PART_NAME[next])} →</button>`
        : finished ? '<button type="button" class="btn primary" data-act="view" data-view="results">See your results →</button>' : '<button type="button" class="btn primary" data-act="view" data-view="exam">Exam overview →</button>');
    renderExamHeader();
    renderNav();
  }
  ...
  function renderExamHome() {
    ...
    const main = ex.finished
      ? `<button type="button" class="btn primary" data-act="view" data-view="results">See your results</button><button type="button" class="btn" data-act="new-exam">Start Practice Exam No. ${ex.no + 1}</button>`
      : `<button type="button" class="btn primary" data-act="resume">${fresh ? 'Start the exam' : 'Continue at question ' + (nxt ? nxt.n : 1)}</button>`;
    const cards = [1, 2, 3].map((p) => {
      ...
      return `<article class="pcard"><div class="pc-h"><span class="pc-n">Part ${PART_NUM[p]}</span><h2>${esc(PART_NAME[p])}</h2></div>
        ...
        <button type="button" class="btn${done && done < parts && !ex.finished ? ' primary' : ''}" data-act="goto-part" data-part="${p}">${ex.finished ? 'Review' : done ? 'Open' : 'Start'} Part ${PART_NUM[p]}</button></article>`;
    }).join('');
    byId('exam-home').innerHTML = `<header class="exam-h"><div class="exam-course">Algebra placement exam</div><h1 tabindex="-1">Practice Exam No. ${esc(ex.no)}</h1>
        <div class="exam-meta">Generated ${esc(started.toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }))} · ${App.items.length} questions · ${sc.total} points${ex.finished ? ' · finished' : ''}</div></header>
  ...
  function renderResults() {
    const ex = examState();
    if (!ex.finished) { byId('results-page').innerHTML = '<p>This exam isn’t finished yet.</p><button type="button" class="btn primary" data-act="view" data-view="exam">Exam overview</button>'; return; }
    ...
    const byPart = [1, 2, 3].map((p) => {
      ...
      return `<tr><td>Part ${PART_NUM[p]}: ${esc(PART_NAME[p])}</td><td><b>${earned}</b> / ${pts}</td><td><button type="button" class="btn ghost" data-act="goto-part" data-part="${p}">Review</button></td></tr>`;
    }).join('');
    byId('results-page').innerHTML = `<section class="results" aria-live="polite"><div class="res-score"><span class="big">${sc.earned}</span><span class="of">/ ${sc.total}</span><span class="pc">${sc.pct}%</span></div>
        ...
        <div class="res-acts"><button type="button" class="btn primary" data-act="new-exam">Generate a new exam</button><button type="button" class="btn ghost" data-act="view" data-view="home">Home</button></div></div></section>`;
  }
  function renderChip() {
    const ex = examState();
    const sc = scoreOf(App.items, ex.res);
  ...
  function renderTutorial() {
    ...
    let html = `<div class="tut-top">${back}${rv !== 'tutorials' ? `<button type="button" class="btn ghost" data-act="view" data-view="tutorials">All tutorials</button>` : ''}
      ...
      <nav class="tut-tabs" aria-label="Tutorial pages">${TUT_TABS.map(([k, name], n) => `<button type="button" data-act="tut-tab" data-tab="${k}"${App.tutTab === k ? ' aria-current="page"' : ''}><span class="tt-n">${n + 1}</span>${name}${k === 'examples' ? ` <span class="tt-c">${wq.length}</span>` : ''}</button>`).join('')}</nav>`;
    [...REG.keys()].forEach((k) => { if (k[0] === 't') REG.delete(k); });
    const nextTab = (k, label) => `<div class="tut-next"><button type="button" class="btn primary" data-act="tut-tab" data-tab="${k}">${label} →</button></div>`;
    if (App.tutTab === 'lesson') {
      html += `<section class="lesson">${R(t.lesson)}</section>` + nextTab('examples', 'Next: worked examples');
      byId('tut-page').innerHTML = html;
      return;
    }
    if (App.tutTab === 'examples') {
      html += `<h2 class="sub-h">Worked examples <span class="tut-stats">${wq.length} examples</span></h2>
  ...
  function openDeck(spec, mode, from, viewOpts) {
  ...
  // ================= navigation =================
  const VIEWS = ['home', 'exam', 'part', 'results', 'tutorials', 'tutorial', 'flashcards', 'deck', 'grapher', 'progress'];
  const TAB_OF = { part: 'exam', results: 'exam', tutorial: 'tutorials', deck: 'flashcards' };
  // ================= pages and addresses =================
  // Every page has an address after "#", so Back/Forward, reload and bookmarks work:
  ...
  function hashOf(v) {
    switch (v) {
      ...
      case 'part': return '#/exam/part-' + App.part;
      case 'results': return '#/exam/results';
      case 'tutorials': return '#/tutorials';
      case 'tutorial': return '#/tutorials/' + App.tutId + (App.tutTab !== 'lesson' ? '/' + App.tutTab : '');
      case 'flashcards': return '#/flashcards';
      case 'deck': {
        const dk = App.deck;
        if (!dk) return '#/flashcards';
        if (dk.spec.topic) return '#/flashcards/' + dk.spec.topic;
        if (dk.spec.section) return '#/flashcards/section/' + slug(dk.spec.section);
        return dk.mode === 'learning' ? '#/flashcards/review' : '#/flashcards/all';
      }
  ...
  function titleOf(v) {
    ...
    const map = {
      home: '', exam: 'Practice Exam No. ' + ex.no, part: 'Part ' + PART_NUM[App.part] + ': ' + PART_NAME[App.part] + ' · Exam No. ' + ex.no,
      results: 'Results · Exam No. ' + ex.no, tutorials: 'Tutorials', tutorial: t ? t.title + ' · ' + (TUT_TABS.find(([k]) => k === App.tutTab) || ['', 'Lesson'])[1] : 'Tutorial',
    ...
    return (map[v] ? map[v] + ' · ' : '') + 'Placement Exam Prep';
  }
  // read an address into a page; unknown addresses go home
  function routeOf(h) {
    const seg = String(h || '').replace(/^#\/?/, '').split('/').filter(Boolean).map((x) => { try { return decodeURIComponent(x); } catch (e) { return x; } });
    const [a, b, c] = seg;
    if (!a) return { v: 'home' };
    if (a === 'exam') {
      if (!b) return { v: 'exam' };
      if (b === 'results') return { v: 'results' };
      const m = /^part-([123])$/.exec(b);
      return m ? { v: 'part', part: +m[1] } : { v: 'exam' };
    }
    if (a === 'tutorials') {
      if (!b) return { v: 'tutorials' };
      if (!MX.byId[b]) return { v: 'tutorials' };
      return { v: 'tutorial', id: b, tab: c === 'examples' || c === 'practice' ? c : 'lesson' };
    }
    if (a === 'flashcards') {
      if (!b) return { v: 'flashcards' };
      if (b === 'all') return { v: 'deck', spec: { all: 1 }, mode: 'all' };
      if (b === 'review') return { v: 'deck', spec: { all: 1 }, mode: 'learning' };
      if (b === 'section') { const sec = bySection().find(([x]) => slug(x) === c); return sec ? { v: 'deck', spec: { section: sec[0] }, mode: 'all' } : { v: 'flashcards' }; }
      return MX.byId[b] && cardsOf(b).length ? { v: 'deck', spec: { topic: b }, mode: 'all' } : { v: 'flashcards' };
    }
    if (a === 'grapher') return { v: 'grapher', mode: b === 'draw' ? 'draw' : 'graph' };
    if (a === 'progress') return { v: 'progress' };
    return { v: 'home' };
  }
  const scrollMem = {};
  function applyRoute(h, opts = {}) {
    const r = routeOf(h);
    if (r.v === 'part') { App.part = r.part; renderExam(); }
    if (r.v === 'tutorial') { if (App.tutId !== r.id) { App.returnView = 'tutorials'; App.returnItem = -1; } App.tutId = r.id; App.tutTab = r.tab; }
    if (r.v === 'grapher' && MX.Grapher) MX.Grapher.state.mode = r.mode;
    const scrollTo = opts.restore ? scrollMem[h] || 0 : 0;
    if (r.v === 'deck') {
      const same = App.deck && JSON.stringify(App.deck.spec) === JSON.stringify(r.spec);
      if (!same) { openDeck(r.spec, r.mode, null, { fromRoute: true, replace: opts.replace, scrollTo }); return; }
      if (r.mode === 'learning' && App.deck.mode !== 'learning') { App.deck.mode = 'learning'; buildOrder(); }
    }
    setView(r.v, { fromRoute: true, replace: opts.replace || hashOf(r.v) !== h, scrollTo });
  }
  // point the address bar at the page on screen: a new history entry, or (replace) the same one
  function syncAddress(v, replace) {
    const h = hashOf(v);
    const was = App.hash;
    App.hash = h;
    d.title = titleOf(v);
    if (G.location.hash === h) return;
    if (replace || !was) {
      try { G.history.replaceState(G.history.state, '', h); } catch (e) { G.location.replace(h); }
    } else G.location.hash = h;
  }
  function setView(v, opts = {}) {
    if (App.hash) scrollMem[App.hash] = G.scrollY; // remember where the page being left was scrolled
    closeStaleFocus();
    App.view = v;
    ...
    if (v === 'grapher' && MX.Grapher) MX.Grapher.render(byId('gr-page'), opts.fromRoute ? { mode: MX.Grapher.state.mode } : undefined);
    syncAddress(v, opts.replace);
    ...
    if (App.booted && !opts.keepFocus) {
      const h1 = byId('v-' + v).querySelector('h1');
  ...
  MX.onGrapherMode = () => { if (App.view === 'grapher') syncAddress('grapher'); };
  ...
  function gotoItem(i) {
    ...
    if (App.view !== 'part' || App.part !== p) { App.part = p; renderExam(); setView('part', { keepFocus: true }); }
  ...
  function onClick(e) {
    ...
    if (act === 'check') doCheck(b.dataset.card, +b.dataset.p);
      ...
      App.tutTab = 'lesson';
      if (App.view !== 'tutorial') { App.returnView = App.view; App.returnItem = App.view === 'part' && b.dataset.item != null ? +b.dataset.item : -1; }
      setView('tutorial');
    } else if (act === 'back') {
      const v = App.returnView || 'tutorials', i = App.returnItem;
      if (v === 'part') { setView('part', { scrollTo: scrollMem[hashOf('part')] || 0 }); if (i != null && i >= 0) setTimeout(() => gotoItem(i), 30); }
      else setView(v, { scrollTo: scrollMem[hashOf(v)] || 0 });
    }
    else if (act === 'view') {
      setView(b.dataset.view);
    }
    else if (act === 'resume') { const i = firstOpenItem(); App.part = App.items[i] ? examPartOf(App.items[i].topic) : 1; renderExam(); setView('part'); if (i > 0) setTimeout(() => gotoItem(i), 30); }
    else if (act === 'tut-tab') { App.tutTab = b.dataset.tab; setView('tutorial'); }
    else if (act === 'goto') gotoItem(+b.dataset.item);
    else if (act === 'to-practice') { App.tutTab = 'practice'; setView('tutorial'); }
    else if (act === 'goto-part') { App.part = +b.dataset.part || 1; renderExam(); setView('part'); }
    else if (act === 'more') moreExercises();
    else if (act === 'new-exam') { startNewExam(); App.part = 1; renderExam(); setView('exam'); }
    ...
    // flashcards
      ...
      if (b.dataset.mode === 'learning' && b.dataset.all) { App.deck = null; }
      openDeck(b.dataset.topic ? { topic: b.dataset.topic } : b.dataset.section ? { section: b.dataset.section } : { all: 1 }, b.dataset.mode || 'all', from);
    }
    else if (act === 'deck-back') { const v = App.deckReturn === 'tutorial' && App.tutId ? 'tutorial' : App.deckReturn === 'home' ? 'home' : 'flashcards'; setView(v, { scrollTo: scrollMem[hashOf(v)] || 0 }); }
    ...
    else if (act === 'deck-mode') { App.deck.mode = b.dataset.mode; buildOrder(); renderDeck(); syncAddress('deck', true); }
  ...
  function boot() {
    ...
    renderExam();
    if (G.history && 'scrollRestoration' in G.history) G.history.scrollRestoration = 'manual';
    applyRoute(G.location.hash || '#/', { replace: true });
    App.booted = true;
    let st = 0;
    G.addEventListener('scroll', () => { clearTimeout(st); st = setTimeout(() => { if (App.hash) scrollMem[App.hash] = G.scrollY; }, 80); }, { passive: true });
    G.addEventListener('hashchange', () => { if (G.location.hash !== App.hash) applyRoute(G.location.hash, { restore: true }); });
  }
