...
# standalone server with its static files) and the tests of the built server. release/BUILD ends
# with the 16-hex release id (a hash of release/'s files).
...
# Later steps add Playwright (step 2), the database tests (step 3) and the container image (step 5).
set -euo pipefail
...
echo "--- engine module (../src -> engine/mx.mjs)"; node engine/build.mjs
echo "--- the plain page's tests, against the module"; node engine/test/run-existing.mjs $( (( quick )) && echo --quick )
echo "--- engine unit tests (the model matches app.js; an exam, lessons and cards on the server)"
