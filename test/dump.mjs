...
const files = ['core.js','texmath.js','parse.js','check.js','svg.js','helpers.js'].map(f=>'src/'+f).concat(readdirSync(join(root,'src/topics')).filter(f=>f.endsWith('.js')).sort().map(f=>'src/topics/'+f));
